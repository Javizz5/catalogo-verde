...
export default function Admin() {
  ...
  const cargarDatos = async () => {
    try {
      const [resProd, resConf, resCat] = await Promise.all([
        fetch(`${API_URL}/productos`), fetch(`${API_URL}/config`), fetch(`${API_URL}/categorias`)
      ]);
  ...
  const manejarLogin = (e) => {
    e.preventDefault();
    if (password === "admin123") { setIsAuthenticated(true); setPassword(''); } 
  ...
  // Toggles the color selection in the array
  ...
  const guardarCambios = async () => {
    ...
    const metodo = productoAEnviar.id ? 'PUT' : 'POST';
    const url = productoAEnviar.id ? `${API_URL}/productos/${productoAEnviar.id}` : `${API_URL}/productos`;
    
    if (metodo === 'PUT') setProductos(productos.map(p => p.id === productoAEnviar.id ? productoAEnviar : p));
    else setProductos([...productos, { ...productoAEnviar, id: 'temp-' + Date.now() }]);
    
    setProductoEditando(null); 
    mostrarNotificacion("Guardando en la nube...");

    try {
      await fetch(url, { method: metodo, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(productoAEnviar) });
      cargarDatos(); 
      mostrarNotificacion("¡Guardado exitoso!");
    } catch (error) { cargarDatos(); }
  };
  ...
  const guardarConfiguracionGlobal = async (nuevoWp, nuevasCats) => {
    try {
      if (nuevoWp !== null) {
        setWhatsapp(nuevoWp);
        await fetch(`${API_URL}/config`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ whatsapp: nuevoWp }) });
      }
      if (nuevasCats !== null) {
        setCategorias(nuevasCats);
        await fetch(`${API_URL}/categorias`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lista: nuevasCats }) });
      }
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-emerald-900 flex items-center justify-center p-4 font-sans">
        <form onSubmit={manejarLogin} className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md text-center">
          <div className="w-20 h-20 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center mx-auto mb-6"><Lock size={40} /></div>
          <h1 className="text-2xl font-black text-gray-800 mb-2">Panel de Administración</h1>
          <input type="password" placeholder="Contraseña" autoComplete="off" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl mb-6 outline-none focus:border-emerald-500 font-bold text-center text-gray-800 mt-6" value={password} onChange={(e) => setPassword(e.target.value)} />
          <button type="submit" className="w-full bg-emerald-700 text-white p-4 rounded-2xl font-bold hover:bg-emerald-800 transition-all">Entrar al Panel</button>
        </form>
      </div>
    );
  }
    ...
    <>
      ...
      {modalBorrar.visible && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl p-8 w-full max-w-sm text-center transform transition-all">
            ...
            <p className="text-gray-500 text-sm font-medium mb-8">Esta acción es permanente y no se podrá recuperar la información.</p>
            <div className="flex gap-3">
              ...
              <button onClick={confirmarEliminacion} className="flex-1 bg-red-500 text-white font-bold py-3.5 rounded-xl hover:bg-red-600 shadow-lg shadow-red-500/20 transition-all">Sí, Eliminar</button>
            </div>
          </div>
        </div>
      )}
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-12 font-sans text-gray-800 relative">
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
          <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
            <button onClick={() => navigate('/')} className="flex items-center gap-2 text-emerald-700 font-bold hover:bg-emerald-50 px-3 py-2 rounded-xl transition-all">
              <ArrowLeft size={20} /> <span className="hidden sm:inline">Catálogo</span>
            </button>
            <div className="flex items-center gap-2">
              <LayoutDashboard size={20} className="text-emerald-600 hidden sm:block" />
              <h1 className="font-black text-gray-800 tracking-tight uppercase text-xs sm:text-sm">Panel Administrativo</h1>
            </div>
            <button onClick={nuevoProducto} className="bg-emerald-700 text-white p-2 sm:px-4 sm:py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-emerald-800 transition-all shadow-md shadow-emerald-200">
        ...
        <main className="max-w-6xl mx-auto px-4 mt-6 sm:mt-10 space-y-6">
          ...
          {/* BLOQUES DE CONFIGURACIÓN SUPERIOR */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-emerald-100 p-6 flex flex-col justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="font-bold text-gray-800 text-sm">WhatsApp de Ventas</h2>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-0.5">Recibir pedidos</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-emerald-100 p-6 flex flex-col justify-between gap-4">
              <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  <h2 className="font-bold text-gray-800 text-sm">Gestor de Categorías</h2>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-0.5">Organiza tu catálogo</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <input type="text" value={nuevaCatStr} onChange={(e) => setNuevaCatStr(e.target.value)} placeholder="Ej: Laptops, Audífonos..." className="w-full p-3 bg-gray-50 border-2 border-transparent rounded-xl outline-none focus:bg-white focus:border-emerald-500 font-bold text-gray-700 text-sm" />
                ...
                <tbody className="divide-y divide-gray-100">
                  {productos.map((prod) => (
                    <tr key={prod.id} className="hover:bg-emerald-50/20 transition-colors">
                      ...
                      <td className="px-6 py-4"><div><p className="font-bold text-gray-800 text-sm">{prod.nombre}</p><p className="text-[10px] text-emerald-600 font-black uppercase mt-0.5">{prod.marca} • {prod.categoria || 'Gen'}</p></div></td>
                      ...
                      <td className="px-6 py-4 text-right"><div className="flex justify-end gap-1"><button onClick={() => setProductoEditando(prod)} className="p-3 text-emerald-600 hover:bg-emerald-100 rounded-2xl transition-all"><Edit size={20} /></button><button onClick={() => solicitarEliminar(prod.id)} className="p-3 text-red-400 hover:bg-red-50 rounded-2xl transition-all"><Trash2 size={20} /></button></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    <div className="min-h-screen bg-gray-50 pb-12 font-sans relative">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 px-4">
        <div className="max-w-5xl mx-auto h-16 flex items-center justify-between">
          <button onClick={() => setProductoEditando(null)} className="flex items-center gap-2 text-gray-500 font-bold hover:text-emerald-700 transition-all p-2 rounded-xl"><ArrowLeft size={20} /> <span className="hidden sm:inline">Cancelar</span></button>
          <div className="text-center"><p className="text-[9px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-0.5">{productoEditando.id ? 'Editando' : 'Creando'}</p><h1 className="font-bold text-gray-800 truncate text-sm sm:text-base leading-none">{productoEditando.nombre || 'Nuevo Producto'}</h1></div>
          <button onClick={guardarCambios} className="bg-emerald-700 text-white px-5 py-2.5 rounded-2xl font-bold flex items-center gap-2 hover:bg-emerald-800 shadow-lg shadow-emerald-700/20 transition-all text-sm"><Save size={18} /> <span className="hidden sm:inline">Guardar</span></button>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 mt-6 sm:mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-emerald-100 text-center">
            <label className="block text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-4">Fotografía</label>
            <div className="aspect-square bg-emerald-50/50 rounded-3xl border-2 border-dashed border-emerald-200 flex flex-col items-center justify-center overflow-hidden relative group transition-all">
              {productoEditando.foto ? (
                <><img src={productoEditando.foto} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-emerald-900/70 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-3 transition-all p-4"><button onClick={() => camaraRef.current.click()} className="w-full bg-white text-emerald-800 text-xs font-bold py-2.5 rounded-xl">Usar Cámara</button><button onClick={() => galeriaRef.current.click()} className="w-full bg-emerald-700 text-white text-xs font-bold py-2.5 rounded-xl">Abrir Galería</button></div></>
              ) : (
                <div className="flex flex-col gap-3 w-full px-6"><button onClick={() => camaraRef.current.click()} className="w-full bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors"><Camera size={18} /> Cámara</button><button onClick={() => galeriaRef.current.click()} className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors"><ImageIcon size={18} /> Galería</button></div>
              )}
            </div>
          ...
          <div className="bg-white p-6 sm:p-10 rounded-[2.5rem] shadow-sm border border-emerald-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
              <div className="sm:col-span-2">
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">Nombre y Marca</label>
                <div className="flex gap-2">
                  <input type="text" placeholder="Nombre" value={productoEditando.nombre} onChange={(e) => setProductoEditando({...productoEditando, nombre: e.target.value})} className="w-2/3 p-4 bg-gray-50 border-2 border-transparent rounded-2xl outline-none focus:bg-white focus:border-emerald-500 font-bold text-gray-800 text-sm" />
              ...
              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">Categoría y Stock</label>
                <div className="flex gap-2 flex-col">
                  ...
                  <div className="relative"><Hash size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input type="number" placeholder="Stock" value={productoEditando.stock} onChange={(e) => setProductoEditando({...productoEditando, stock: e.target.value})} className="w-full pl-9 p-3.5 bg-gray-50 border-2 border-transparent rounded-2xl outline-none focus:bg-white focus:border-emerald-500 font-bold text-gray-800 text-sm" /></div>
                </div>
              </div>
            </div>
            ...
            <div className="mb-10">
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">Colores Disponibles (Opcional)</label>
            ...
            <div className="flex items-center gap-4 mb-6"><h2 className="text-[11px] font-black text-emerald-700 uppercase tracking-widest flex items-center gap-2"><DollarSign size={14} /> Gestión de Precios ($)</h2><div className="h-px w-full bg-gray-100"></div></div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="group"><label className="block text-[10px] font-black text-emerald-700 uppercase mb-2 ml-2">Total a Crédito</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold">$</span><input type="number" value={productoEditando.credito} onChange={(e) => setProductoEditando({...productoEditando, credito: e.target.value})} className="w-full pl-8 p-3.5 bg-white border-2 border-gray-100 rounded-2xl outline-none focus:border-emerald-500 font-black text-gray-800" /></div></div>
              ...
              <div className="space-y-4">
                <div className="group"><label className="block text-[10px] font-black text-emerald-700 uppercase mb-2 ml-2">Monto de Inicial</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold">$</span><input type="number" value={productoEditando.inicial} onChange={(e) => setProductoEditando({...productoEditando, inicial: e.target.value})} className="w-full pl-8 p-3.5 bg-white border-2 border-gray-100 rounded-2xl outline-none focus:border-emerald-500 font-black text-gray-800" /></div></div>
